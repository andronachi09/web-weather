...
function CityCard() {
  const weather = useContextSelector(WeatherContext, (state) => state.weather);
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col">
            <div className="flex flex-row justify-between">
              ...
              <div>
                <img
                  src={`https://openweathermap.org/img/wn/${weather?.weatherDescription[0].icon}@2x.png`}
                  alt={weather?.weatherDescription[0].description}
                />
              </div>
            </div>
            <div className="flex flex-row justify-center space-x-10 mb-8">
              <div className="flex flex-col pt-2">
                <h2 className="text-2xl text-gray-200 m-1">
                  {Math.round(weather?.temperature.current as number)}°
                </h2>
